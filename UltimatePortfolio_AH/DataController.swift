...
/// counting fetch request, traking orders, and dealing with simple data.
class DataController: ObservableObject {
    ...
    /// or no permanent storage (for use in regular app runs.)
    ///
    ...
    /// Runs a fetch request with various predicaets that filterthe user's issues based on
    ...
    func issuesForSelectedFilter() -> [Issue] {
        ...
        let filter = selectedFilter ?? .all // Filtro actual o "all"
        ...
        if filterTokens.isEmpty == false {
            for filterToken in filterTokens {
                let tokenPredicate = NSPredicate(format: "tag CONTAINS %@", filterToken)
